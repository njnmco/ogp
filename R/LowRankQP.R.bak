...
# Fixes niter bug
# From LowRankQP Package
LowRankQP <- function (Vmat, dvec, Amat, bvec, uvec, method = "PFCF", verbose = FALSE,
    niter = 200)
{
    typeError <- FALSE
    ...
    if (typeError)
        stop("ERROR: check input dimensions.")
    n <- nrow(Vmat)
    m <- ncol(Vmat)
    p <- nrow(Amat)
    alpha <- as.array(matrix(0, n, 1))
    beta <- as.array(matrix(0, p, 1))
    xi <- as.array(matrix(0, n, 1))
    zeta <- as.array(matrix(0, n, 1))
    if (method == "LU")
        methodNum <- 1
    if (method == "CHOL")
        methodNum <- 2
    ...
    res <- .C("LowRankQP", n, m, p, as.integer(methodNum), as.integer(verbose),
        as.integer(niter), Vmat, dvec, t(Amat), bvec, uvec, alpha,
        beta, xi, zeta, PACKAGE = "LowRankQP")
    alpha <- res[[12]]
    beta <- res[[13]]
    xi <- res[[14]]
    zeta <- res[[15]]
